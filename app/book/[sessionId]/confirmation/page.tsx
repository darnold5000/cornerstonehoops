...
export const metadata = createMetadata({
  ...
  description: "Your Cornerstone Hoops reservation confirmation.",
    ...
    <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6">
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        ...
        <Button asChild variant="outline">
          <a
            href={`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(session.title)}&dates=${session.session_date.replace(/-/g, "")}T${session.start_time.replace(/:/g, "").slice(0, 6)}/${session.session_date.replace(/-/g, "")}T${session.end_time.replace(/:/g, "").slice(0, 6)}&details=${encodeURIComponent("Cornerstone Hoops")}&location=${encodeURIComponent(session.location_address ?? SITE.address.full)}`}
            target="_blank"
