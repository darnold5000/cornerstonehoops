...
export async function generateMetadata({
  ...
  if (!session)
    return createMetadata({
      title: "Session",
      description: "Training session details at Cornerstone Hoops.",
      path: `/schedule/${sessionId}`,
    });
  return createMetadata({
    title: session.title,
    description: session.description ?? `Book ${session.title} at Cornerstone Hoops.`,
    path: `/schedule/${sessionId}`,
  });
}
    ...
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 md:py-16">
      ...
      <dl className="mt-8 grid gap-4 rounded-xl border border-border bg-card p-6 sm:grid-cols-2">
        ...
        <div>
          ...
          <dd className="font-medium">{session.trainer?.name ?? "Cornerstone staff"}</dd>
        </div>
        <div>
