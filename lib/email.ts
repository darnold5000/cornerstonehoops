...
export async function sendBookingConfirmation(payload: ConfirmPayload): Promise<void> {
  ...
  await resend.emails.send({
    ...
    subject: `Cornerstone Hoops booking confirmed — ${payload.sessionTitle}`,
    html: `
      <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto;">
        <h1 style="color: #121212;">You're booked at Cornerstone Hoops</h1>
...
export async function sendStaffBookingNotification(payload: StaffPayload): Promise<void> {
  ...
  await resend.emails.send({
    from,
    to: staffEmail,
    subject: `New Cornerstone Hoops booking — ${payload.sessionTitle}`,
    html: `
...
export async function sendWaitlistConfirmation(payload: {
  ...
  await resend.emails.send({
    from,
    to: payload.email,
    subject: "You're on the Cornerstone Hoops waitlist",
    html: `
      <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto;">
