...
export function BookingForm({
  ...
  useEffect(() => {
    ...
    /* Hydrate returning-family details from localStorage after mount. */
    /* eslint-disable react-hooks/set-state-in-effect */
    setSavedFamily(family);
    setUseSaved(true);
    setSelectedAthleteId(family.athletes[0].id);
    setForm((prev) => ({
      ...prev,
      ...
      athleteFirstName: family.athletes[0].firstName,
      athleteLastName: family.athletes[0].lastName,
      athleteDob: family.athletes[0].dob,
      primarySport: family.athletes[0].primarySport ?? "",
      experienceLevel: family.athletes[0].experienceLevel ?? "",
      medicalNotes: family.athletes[0].medicalNotes ?? "",
      customerNotes: "",
    }));
    ...
    <form onSubmit={onSubmit} className="space-y-8">
      ...
      <fieldset className="space-y-3 rounded-xl border border-border p-4">
        ...
        <label className="flex items-start gap-3 text-sm">
          ...
          />
          <span>
            I acknowledge liability waiver language will be provided by Cornerstone Hoops
